...
class SJISPipeline:
    ...
    def extract_lines(self, img_rgb, text_lines, method, threshold, line_thickness, kmeans_k, invert_output):
        ...
        if method == "Segmentation (K-means)":
            ...
            # [새로 추가된 XDoG, Laplacian, Sobel 엣지 추출기]
            if method == "XDoG (Soft Sketch)":
                ...
                # Threshold(0~255)를 XDoG의 tau(-5.0~5.0)로 매핑
    ...
    def process_thinning(self, binary_img, clean_strength, method):
        ...
        # [핵심] 픽셀 면적(Connected Components) 기반 노이즈 제거
        if clean_strength > 0:
            min_area = int(clean_strength * 10) # 예: 1.5 -> 15픽셀 미만 덩어리 제거
            if min_area > 0:
    ...
    def _get_gap_string(self, target_w, is_start, last_char='', exact_match=False):
        ...
        max_fw = int(target_w // fw_w) + 1
        for fw in range(max_fw, -1, -1):
            rem1 = target_w - fw * fw_w
            max_hw = int(max(0, rem1) // hw_w) + 1
            for hw in range(max_hw, -1, -1):
                rem2 = rem1 - hw * hw_w
                dt = int(round(rem2 / dt_w)) if rem2 > 0 else 0
                    
                for d in [max(0, dt-1), dt, dt+1]:
                    ...
                    cost = fw * 1 + hw * 5 + d * 15 
                    total_score = diff * diff_penalty + cost
        ...
        total_chars = c_fw + c_hw + c_dt
        res = ""
        
        for _ in range(total_chars):
            score_fw = c_fw * 1.0 + (0.5 if res and res[-1] != '　' else 0)
            score_dt = c_dt * 1.1 + (0.5 if res and res[-1] != '.' else 0)
            ...
            if can_fw and score_fw > best_s: best_choice = '　'; best_s = score_fw
            if can_dt and score_dt > best_s: best_choice = '.'; best_s = score_dt
            if can_hw and score_hw > best_s: best_choice = ' '; best_s = score_hw
            ...
            if best_choice == '　':
                ...
                if c_hw > 0: res += ' '; c_hw -= 1
                elif c_dt > 0: res += '.'; c_dt -= 1
                elif c_fw > 0: res += '　'; c_fw -= 1
    ...
    def _solve_stripe_score_priority(self, score_matrix, char_data_list, w, spacing, last_ink_x, is_roi_mask, prev_anchors):
        ...
        if cx < w - self._dot_width:
            gs, gs_w = self._get_gap_string(w - cx, isl, last_char, exact_match=False)
            if gs:
                line += gs
                cx_temp = cx
                for gc in gs:
...
class SJISApp(QMainWindow):
    ...
    def save_image(self, label_widget, default_name):
        if label_widget._pixmap and not label_widget._pixmap.isNull():
            fname, _ = QFileDialog.getSaveFileName(self, "Save Image", default_name, "PNG Files (*.png);;JPEG Files (*.jpg);;All Files (*)")
            if fname:
    ...
    def initUI(self):
        self.setWindowTitle("SJIS-Art Generator (Portable V16 - Advanced Edge & Filters)")
        ...
        self.combo_line.addItems(["Adaptive Threshold", "Segmentation (K-means)", "Laplacian (Brush/Ink)", "Sobel (Directional Edge)", "XDoG (Soft Sketch)", "Canny (Hard Edges)", "Simple (Grayscale)"])
        ...
        self.combo_thin.addItems(["KMM", "K3M", "Guo-hall"])
        ...
        self.combo_place.addItems(["Sequential", "Score-Priority"])
        ...
        self.spin_dot_pen = SliderSpinBox("Dot Pen:", 0.0, 20.0, 0.1, 5.0)
        ...
        self.spin_yp_pen = SliderSpinBox("Y-Shift Pen:", 0.0, 10.0, 0.1, 0.5)
        ...
        self.spin_eye_w = SliderSpinBox("Eye Char W:", 0.0, 500.0, 1.0, 100.0)
        self.spin_roi_w = SliderSpinBox("Wide Char W:", 0.0, 50.0, 1.0, 15.0)
        self.spin_roi_pha = SliderSpinBox("ROI Phase:", 0.0, 5.0, 0.1, 0.0)
        self.spin_roi_den = SliderSpinBox("ROI Density:", 0.0, 2.0, 0.05, 0.0)
        self.spin_roi_mis = SliderSpinBox("ROI Missing:", 0.0, 2.0, 0.05, 0.0)
        self.spin_roi_frq = SliderSpinBox("ROI Freq:", 0.0, 10.0, 0.1, 1.0)
        ...
        self.spin_brush = SliderSpinBox("Brush Size:", 1, 100, 1, 20, False)
        ...
        self.text_custom_chars = QTextEdit()
        ...
        self.spin_custom_pen = SliderSpinBox("Custom Pen:", 0.0, 20.0, 0.5, 5.0)
        ...
        w6_lay = QVBoxLayout(); w6_lay.addWidget(QLabel("6. Output (Selectable Text)"))
    ...
    def load_image(self):
        fname, _ = QFileDialog.getOpenFileName(self, "Open Image", "", "Image Files (*.png *.jpg *.jpeg *.bmp *.webp)")
        if fname:
